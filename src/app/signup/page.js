...
export default function SignupPage() {
  ...
  const validate = () => {
    ...
    if (form.password.length < 8) e.password = "رمز حداقل ۸ کاراکتر باشه.";
    setErrors(e);
  ...
  const submit = async (ev) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409) {
          setServerError("این ایمیل قبلاً عضوِ بَده. برو وارد شو.");
          document.getElementById("signup-email")?.focus();
        } else {
          setServerError(data?.error || "یه چیزی این وسط قاطی کرد. دوباره امتحان کن.");
        }
        setPending(false);
        return;
      }
      // hard nav so CartProvider remounts + re-fetches /api/cart (folds the guest
      // cart into the new account) — same reasoning as login.
  ...
  return (
    <AuthShell heading="بد، یکی از ما شو" sub="حساب بساز تا سفارش‌هات همیشه دمِ دستت باشه.">
      ...
      <form onSubmit={submit} noValidate className="space-y-4">
        ...
        <PasswordField
          ...
          hint="حداقل ۸ کاراکتر. یه چیزی که خودت یادت بمونه."
          value={form.password}
          onChange={set("password")}
          error={errors.password}
        />
        <SubmitButton pending={pending} idle="بزن بریم" busy="داره ساخته می‌شه…" />
      </form>
